...
#include "Test/Public/TestModuleActor.h"
...
AModuleAndPluginCharacter::AModuleAndPluginCharacter()
{
	...
	CharacterData->CharacterBaseInfo.CharacterName = TEXT("Test");
	CharacterData->CharacterBaseInfo.Level = 10;
	CharacterData->CharacterBattleInfo.MaxHealth = 500.0f;
...
void AModuleAndPluginCharacter::BeginPlay()
{
	...
	UCharacterDataHelper::PrintCharacterData(CharacterData);
}

void AModuleAndPluginCharacter::TestSpawnActor()
{
	FVector Location = GetActorLocation() + FVector(100.f, 100.f, 100.f);

	GetWorld()->SpawnActor<ATestModuleActor>(TestModuleActor, Location, FRotator::ZeroRotator);
}
